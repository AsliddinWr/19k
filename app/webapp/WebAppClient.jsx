...
function AdminView(props) {
  ...
  const catalogList = (giftCatalog || []).filter((gift) => {
    ...
    return String(gift.title || '').toLowerCase().includes(query) || String(gift.telegram_gift_id || '').toLowerCase().includes(query);
  });
      ...
      {adminTab === 'catalog' ? (
        <div className="screen-stack">
          ...
          <div className="premium-card catalog-search-card">
            ...
            <small>Jami catalog: {giftCatalog.length} ta · Ko‘rinayotgan: {catalogList.length} ta</small>
          </div>
          ...
          <div className="gift-catalog-grid">
            {catalogList.map((gift) => (
              ...
              >
                <span className="catalog-resale-ribbon">gift</span>
                <div className="catalog-gift-art"><GiftMedia gift={gift} preferStatic /></div>
                <strong>{gift.title}</strong>
