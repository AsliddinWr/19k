...
function giftPriceLabel(gift = {}) {
  const price = gift.display_price || gift.floor_price || gift.star_price || gift.manual_price || 0;
  if (!price) return 'No price';
  const currency = String(gift.price_currency || '').toUpperCase();
  if (currency === 'TON') return `${formatPrice(price)} TON`;
  if (currency === 'STARS' || !currency) return `${formatPrice(price)} ⭐`;
...
export default function WebAppClient() {
  ...
  const [marketGiftForm, setMarketGiftForm] = useState({ source: 'giftasset', giftNames: '', pages: '3', limit: '20', json_url: '' });
      ...
      <section className="app-main">
        ...
        {tab === 'admin' && isAdmin ? (
          <AdminView
            ...
            marketGiftForm={marketGiftForm}
            setMarketGiftForm={setMarketGiftForm}
            ...
            syncMarketNftGifts={syncMarketNftGifts}
...
function isTgsAnimation(url = '', gift = {}) {
  ...
  return mediaType === 'tgs' || mediaType === 'lottie' || cleanUrl.endsWith('.tgs') || cleanUrl.endsWith('.json') || cleanUrl.endsWith('.lottie');
}
...
function catalogKindLabel(gift = {}) {
  if (catalogGiftKind(gift) !== 'nft') return 'Regular';
  const source = String(gift?.source || gift?.price_source || '').toLowerCase();
  return source && source !== 'telegram' ? 'Market NFT' : 'NFT / Animated';
}
...
function AdminView(props) {
  const {
    adminTab, setAdminTab, cases, gifts, giftsByCase, giftCatalog, caseRewardForm, setCaseRewardForm, catalogSearch, setCatalogSearch, marketGiftForm, setMarketGiftForm, adminUsers, adminWithdrawals,
    ...
    syncTelegramGifts, syncMarketNftGifts, createCaseReward, updateCaseReward, deleteCaseReward,
  ...
  const catalogList = (giftCatalog || []).filter((gift) => {
    ...
    return String(gift.title || '').toLowerCase().includes(query) ||
      ...
      String(gift.collection_name || '').toLowerCase().includes(query) ||
      String(gift.model_name || '').toLowerCase().includes(query) ||
      String(gift.external_id || '').toLowerCase().includes(query);
  });
        ...
        <div className="screen-stack">
          <div className="toolbar premium-card catalog-toolbar">
            ...
            <div className="catalog-action-buttons">
              ...
              <button className="ghost-btn" type="button" disabled={busy} onClick={syncMarketNftGifts}><AppIcon name="gem" /> Sync Market NFT</button>
            </div>
          </div>

          <div className="premium-card nft-market-sync-card">
            <div>
              <strong>NFT marketplace giftlar</strong>
              <span>SnoopDogg kabi t.me/nft/... collectible giftlar Bot API’dan emas, marketplace/data source’dan keladi. Bu joy barcha market NFTlarni catalogga ketma-ket qo‘shadi.</span>
            </div>
            <div className="market-sync-grid">
              <Select label="Source" value={marketGiftForm.source} onChange={(value) => setMarketGiftForm({ ...marketGiftForm, source: value })}>
                <option value="giftasset">GiftAsset API</option>
                <option value="portals">Portals API</option>
                <option value="json">Custom JSON URL</option>
              </Select>
              <Input label="Collection / NFT nomlari" value={marketGiftForm.giftNames} onChange={(value) => setMarketGiftForm({ ...marketGiftForm, giftNames: value })} placeholder="Snoop Dogg, Toy Bear yoki SnoopDogg-232372" />
              <Input label="Pages" type="number" value={marketGiftForm.pages} onChange={(value) => setMarketGiftForm({ ...marketGiftForm, pages: value })} placeholder="3" />
              <Input label="Limit" type="number" value={marketGiftForm.limit} onChange={(value) => setMarketGiftForm({ ...marketGiftForm, limit: value })} placeholder="20" />
