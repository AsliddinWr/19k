...
function fileExtension(path = '', contentType = '') {
  ...
  if (contentType.includes('gzip') || contentType.includes('x-tgsticker') || contentType.includes('tgs')) return 'tgs';
  return 'bin';
}
...
function mediaTypeFromSticker(sticker = {}, extension = '') {
  if (sticker.is_video || extension === 'webm') return 'webm';
  if (sticker.is_animated || extension === 'tgs') return 'tgs';
  return 'image';
}

function giftKindFromSticker(gift = {}, sticker = {}) {
  if (gift.type === 'unique' || gift.is_unique || gift.upgrade_star_count) return 'nft';
  if (sticker.is_video || sticker.is_animated || sticker.premium_animation) return 'nft';
  return 'regular';
}
...
async function mirrorTelegramFile(supabase, fileId, targetPrefix) {
  if (!fileId) return { publicUrl: null, extension: null, contentType: null };
  ...
  if (!fileInfo?.file_path) return { publicUrl: null, extension: null, contentType: null };

  const fileUrl = `https://api.telegram.org/file/bot${token}/${fileInfo.file_path}`;
  const fileResponse = await fetch(fileUrl, { cache: 'no-store' });

  if (!fileResponse.ok) return { publicUrl: null, extension: null, contentType: null };
  ...
  return { publicUrl: data?.publicUrl || null, extension: ext, contentType };
}

function giftTitle(gift, index) {
  if (gift.title) return gift.title;
...
export async function POST(request) {
  try {
    ...
    for (let index = 0; index < gifts.length; index += 1) {
      ...
      const thumbnailFileId = sticker.thumbnail?.file_id || sticker.thumb?.file_id || null;
      const mainStickerFileId = sticker.file_id || null;

      try {
        if (thumbnailFileId) {
          const mirrored = await mirrorTelegramFile(supabase, thumbnailFileId, `telegram/${telegramGiftId}/thumb`);
      ...
      try {
        if (mainStickerFileId) {
          const mirrored = await mirrorTelegramFile(supabase, mainStickerFileId, `telegram/${telegramGiftId}/sticker`);
          const detectedMediaType = mediaTypeFromSticker(sticker, mirrored.extension);
          mediaType = detectedMediaType;

          if (detectedMediaType === 'image') {
            imageUrl = imageUrl || mirrored.publicUrl;
          } else {
            animationUrl = mirrored.publicUrl;
            animated += 1;
          }
        }
      } catch {
      ...
      if (!imageUrl && animationUrl && mediaType === 'webm') imageUrl = animationUrl;

      const starPrice = Number(gift.star_count || gift.stars || 0);
      const giftKind = giftKindFromSticker(gift, sticker);
      const row = {
        telegram_gift_id: telegramGiftId,
        slug,
        title: giftTitle(gift, index),
        type: giftKind === 'nft' ? 'nft_gift' : 'telegram_gift',
        description: giftKind === 'nft' ? 'Animated Telegram gift' : 'Telegram gift',
        image_url: imageUrl,
        animation_url: animationUrl,
        media_type: mediaType,
        gift_kind: giftKind,
        sticker_file_id: sticker.file_id || null,
    ...
    return Response.json({ ok: true, count: synced, animated, gifts: rows });
  } catch (error) {
