...
async function fetchGiftAsset(path, { apiKey, method = 'GET', params = {}, body = null } = {}) {
  if (!apiKey) {
    throw new Error('GIFTASSET_API_KEY topilmadi. Vercel env ga GIFTASSET_API_KEY qo‘ying va redeploy qiling.');
  }

  const url = new URL(path.replace(/^\//, ''), GIFTASSET_API_URL);
  ...
  if (!response.ok) {
    ...
    throw new Error(`GiftAsset API xatosi: ${response.status} ${message}`);
  }

  return data?.result ?? data;
}

function parseGiftTokens(value = '') {
  return cleanText(value)
    .split(',')
    .map((item) => item.trim())
    .filter(Boolean)
    .map((item) => item.replace(/^https?:\/\/t\.me\/nft\//i, '').replace(/^t\.me\/nft\//i, ''));
}

function looksLikeUniqueGiftName(value = '') {
  return /^[A-Za-z][A-Za-z0-9_]*-\d+$/.test(cleanText(value));
}

async function fetchGiftAssetItems({ apiKey, limit, pages, giftNames }) {
  const names = parseGiftTokens(giftNames);
  const all = [];

  const uniqueNames = names.filter(looksLikeUniqueGiftName);
  for (const name of uniqueNames) {
    const gift = await fetchGiftAsset('/api/v1/gifts/get_gift_by_name', {
      apiKey,
      params: { name },
    });
    all.push(gift);
  ...
  const collections = names.filter((name) => !looksLikeUniqueGiftName(name));
  const safePages = Math.min(Math.max(Number(pages || 3), 1), 20);
  const receiver = Number(process.env.GIFTASSET_RECEIVER_ID || 0);

  for (const collectionName of collections.length ? collections : ['All']) {
    for (let page = 0; page < safePages; page += 1) {
      ...
      const data = await fetchGiftAsset('/api/aggregator', {
        apiKey,
        method: 'POST',
        params: { page },
        body: payload,
      ...
      const items = flattenApiItems(data);
      all.push(...items);
      if (!items.length || items.length < Number(limit || 20)) break;
    }
  }

  // fallback: if aggregator does not return individual items, import collection floors as catalog cards
  if (!all.length) {
    const priceList = await fetchGiftAsset('/api/v1/gifts/get_gifts_price_list', {
      apiKey,
      params: { models: 'false', premarket: 'false' },
    });
    const floors = priceList?.collection_floors || {};
    for (const [collection_name, prices] of Object.entries(floors)) {
      all.push({
        telegram_gift_title: collection_name,
        telegram_gift_name: safeSlug(collection_name),
        collection_name,
        market_floor: { min: Math.min(...Object.values(prices).filter((v) => Number.isFinite(Number(v))).map(Number)) || 0 },
        media_preview: `${GIFTASSET_API_URL.replace(/\/$/, '')}/api/v1/data/original/${encodeURIComponent(safeSlug(collection_name).replace(/-/g, ''))}.png`,
        providers: { giftasset: prices },
      });
    }
  }

  return all.slice(0, Math.max(1, Number(limit || 20)) * Math.max(1, Number(pages || 3)) * Math.max(1, collections.length || 1));
}

async function fetchPortalsItems({ authData, limit, pages, giftNames }) {
  if (!authData) {
    throw new Error('PORTALS_AUTH_DATA topilmadi. Marketplace NFT giftlar uchun Vercel env ga Portals Authorization headerini qo‘ying: PORTALS_AUTH_DATA=tma ...');
  }

  const all = [];
  const safeLimit = Math.min(Math.max(Number(limit || 20), 1), 20);
  const safePages = Math.min(Math.max(Number(pages || 3), 1), 10);
  const names = cleanText(giftNames)
    .split(',')
    .map((item) => item.trim())
    .filter(Boolean);

  for (let page = 0; page < safePages; page += 1) {
    const url = new URL('nfts/search', PORTALS_API_URL);
    url.searchParams.set('offset', String(page * safeLimit));
    url.searchParams.set('limit', String(safeLimit));
    url.searchParams.set('sort_by', 'price asc');
    url.searchParams.set('status', 'listed');
    if (names.length) url.search += `&filter_by_collections=${portalsCollectionParam(names)}`;
    ...
    const data = await response.json().catch(() => null);
    if (!response.ok) {
      throw new Error(`Portals API xatosi: ${response.status}. PORTALS_AUTH_DATA muddati tugagan yoki noto‘g‘ri bo‘lishi mumkin.`);
    }

    const items = data?.results || data?.nfts || data?.items || [];
    all.push(...items);
    if (!items.length || items.length < safeLimit) break;
  }
...
export async function POST(request) {
  try {
    ...
    const source = cleanText(body.source || 'giftasset').toLowerCase();
    let items = [];
    ...
    if (source === 'json') {
      if (Array.isArray(body.items)) {
        ...
        if (!jsonUrl) throw new Error('JSON import uchun NFT_GIFTS_JSON_URL env yoki json_url kerak.');
        items = await fetchJsonSource(jsonUrl);
      }
    } else if (source === 'giftasset') {
      items = await fetchGiftAssetItems({
        apiKey: cleanText(body.apiKey || process.env.GIFTASSET_API_KEY || ''),
        limit: body.limit || 20,
        pages: body.pages || 3,
        giftNames: body.giftNames || process.env.GIFTASSET_COLLECTIONS || '',
      });
    } else {
      items = await fetchPortalsItems({
        authData: cleanText(body.authData || process.env.PORTALS_AUTH_DATA || ''),
        limit: body.limit || 20,
        pages: body.pages || 3,
        giftNames: body.giftNames || process.env.PORTALS_COLLECTIONS || '',
      });
    }

    if (!Array.isArray(items) || !items.length) throw new Error('Marketplace’dan NFT gift topilmadi. Source/collection/authni tekshiring.');

    let synced = 0;
    const rows = items.map((item, index) => normalizeMarketGift(item, index, source));
    ...
    return Response.json({ ok: true, count: synced, source, gifts: rows });
  } catch (error) {
    return jsonError(error.message || 'Marketplace NFT sync xatosi', 500);
  }
}
